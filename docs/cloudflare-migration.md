...
移行時、DBが参照する固有画像キー1,191件のうち、1件は移行元にも存在しない不正なキーでした（[Issue #36](https://github.com/shigurenimo/swimmy/issues/36)）。全実画像を元キーのまま移行し、この不正参照は移行後の修復対象として残しました。

2026-09-10、余分な `undefined` を除いたIDが一意に一致することを根拠に、対象投稿1件の画像参照を修復しました。候補画像はR2から読み戻し、元Storageのバックアップとサイズ・SHA-256が一致することを確認しました。変更したのは画像参照だけで、本文・日時などは維持しています。修復前後のデータと切り戻し用SQLは非公開で保管しています。

修復後、D1の固有画像参照1,191件をR2の全1,904件と照合し、参照切れ0件、外部キー違反0件、整合性検査 `ok` を確認しました。Chromeで本番スレッドの添付画像が表示されることも確認済みです。新規投稿では、実在しない画像IDを保存前に拒否する対策を適用しています。
